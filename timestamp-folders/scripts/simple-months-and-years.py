...
def create_folders(base_path, years, structure):
    """
    Creates folders based on the specified structure.
    """
    months = [
    ...
    for year in years:
        ...
        for month_num, month_abbr in months:
            ...
            subfolder_path = os.path.join(year_folder, subfolder_name)
            
            try:
                os.makedirs(subfolder_path, exist_ok=True)
                print(f"Created: {subfolder_path}")
            except Exception as e:
                print(f"Error creating {subfolder_path}: {e}")
...
def main():
    ...
    create_folders(base_path, selected_years, folder_structure)
